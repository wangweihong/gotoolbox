...
func fromError(err error) *WithStack {
	...
	if v, ok := err.(errors.StackTrace); ok {
		stack := v.Stack()
		stack = append(stack, newStack(unknown.code, errors.Caller()))
...
func newStack(code int, caller string) string {
	return errors.CurrentModule.String() + fmt.Sprintf(",code:%d,", code) + caller
}
