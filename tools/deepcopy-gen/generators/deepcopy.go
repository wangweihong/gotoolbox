...
func NewGenDeepCopy(
	sanitizedName, targetPackage string,
	boundingDirs []string,
	allTypes, registerTypes bool,
) generator.Generator {
	return &genDeepCopy{
...
func deepCopyMethod(t *types.Type) (*types.Signature, error) {
	...
	ptrRcvr := f.Signature.Receiver != nil && f.Signature.Receiver.Kind == types.Pointer &&
		f.Signature.Receiver.Elem.Name == t.Name
...
func (g *genDeepCopy) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	if deepCopyMethodOrDie(t) == nil {
		sw.Do(
			"// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new $.type|raw$.\n",
			args,
		)
		if isReference(t) {
	...
	for _, intf := range intfs {
		sw.Do(
			fmt.Sprintf(
				"// DeepCopy%s is an autogenerated deepcopy function, copying the receiver, creating a new $.type2|raw$.\n",
				intf.Name.Name,
			),
			argsFromType(t, intf),
		)
		if nonPointerReceiver {
			sw.Do(
				fmt.Sprintf("func (in $.type|raw$) DeepCopy%s() $.type2|raw$ {\n", intf.Name.Name),
				argsFromType(t, intf),
			)
...
func (g *genDeepCopy) doMap(t *types.Type, sw *generator.SnippetWriter) {
	...
	case uet.Kind == types.Interface:
		...
		if uet.Name.Name == "interface{}" {
			klog.Fatalf(
				"DeepCopy of %q is unsupported. Instead, use named interfaces with DeepCopy<named-interface> as one of the methods.",
				uet.Name.Name,
			)
		}
