...
func (p *DirectoryProcessor) LocateToDisk() *DirectoryProcessor {
	...
	if err := os.MkdirAll(p.LocateParsedDir, 0755); err != nil {
	...
	if err := filepath.Walk(p.TemplateDir, p.parseAndWriteToDisk); err != nil {
		p.err = fmt.Errorf("templateProc %v parseAndWriteToDisk %v to disk %v　fail:%v", p.TemplateName, p.TemplateDir, p.LocateParsedDir, err)
		return p
	}
	if err := filepath.Walk(p.TemplateDir, p.writeYamlToDisk); err != nil {
		p.err = fmt.Errorf("templateProc %v parseAndWriteToDisk %v to located disk %v　fail:%v", p.TemplateName, p.TemplateDir, p.LocateParsedDir, err)
		return p
	}
...
func (p *DirectoryProcessor) parseAndWriteToDisk(path string, info os.FileInfo, err error) error {
	...
	if !info.IsDir() {
		if strings.HasSuffix(info.Name(), ".template") {
			...
			if err := ioutil.WriteFile(yalmPathToDisk, []byte(dataWriteToCache), 0644); err != nil {
				return fmt.Errorf("templateProc %v template file %v save to file %v error :%v", p.TemplateName, path, yalmPathToDisk, err.Error())
			}
		}
	}
...
func (p *DirectoryProcessor) writeYamlToDisk(path string, info os.FileInfo, err error) error {
	...
	if !info.IsDir() {
		...
		if subPath != "" {
			...
			if err := os.MkdirAll(locateDir, 0755); err != nil {
		...
		if err := ioutil.WriteFile(parsedAbsPath, []byte(dataWriteToCache), 0644); err != nil {
			return fmt.Errorf("templateProc %v template file %v save to file %v error :%v", p.TemplateName, path, parsedAbsPath, err.Error())
		}
	}
