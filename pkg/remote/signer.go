...
func signerFromPem(pemBytes []byte, password []byte) (ssh.Signer, error) {
	...
	// read pem block
	err := errors.Errorf("pem decode failed, no key found")
...
func parsePemBlock(block *pem.Block) (interface{}, error) {
	...
	case "RSA PRIVATE KEY":
		...
		if err != nil {
			return nil, errors.Errorf("parsing PKCS private key failed %v", err)
		} else {
			return key, nil
		}
	case "EC PRIVATE KEY":
		...
		if err != nil {
			return nil, errors.Errorf("parsing EC private key failed %v", err)
		} else {
			return key, nil
		}
	case "DSA PRIVATE KEY":
		...
		if err != nil {
			return nil, errors.Errorf("parsing DSA private key failed %v", err)
		} else {
			return key, nil
		}
	default:
