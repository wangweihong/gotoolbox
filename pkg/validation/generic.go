...
// IsQualifiedName tests whether the value passed is what program calls a
...
// IsValidateURLWithPort 检测URL格式是否正确
func IsValidateURLWithPort(addr string) bool {
	// 使用正则匹配带 http(s) 协议头和端口的 URL
	regex := `^http(s)?://([\w.-]+)(:\d+)?(/[\w./?%&=]*)?$`
...
// IsValidatePhoneNumber 检测手机号码格式是否正确
