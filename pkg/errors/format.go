...
//	%-v:   error for internal read B - #0 [github.com/wangweihong/gotoolbox/main.go:12 (main.main)] (#100102) Internal Server Error
//	%+v:   error for internal read B - #0 [github.com/wangweihong/gotoolbox/main.go:12 (main.main)] (#100102) Internal Server Error; error for internal read A - #1 [github.com/wangweihong/gotoolbox/main.go:35 (main.newErrorB)] (#100104) Validation failed
//	%#v:   [{"error":"error for internal read B"}]
//	%#-v:  [{"caller":"#0 github.com/wangweihong/gotoolbox/main.go:12 (main.main)","error":"error for internal read B","message":"(#100102) Internal Server Error"}]
//	%#+v:  [{"caller":"#0 github.com/wangweihong/gotoolbox/main.go:12 (main.main)","error":"error for internal read B","message":"(#100102) Internal Server Error"},{"caller":"#1 github.com/wangweihong/gotoolbox/main.go:35 (main.newErrorB)","error":"error for internal read A","message":"(#100104) Validation failed"}]
...
func format(k int, jsonData []map[string]interface{}, str *bytes.Buffer, finfo *formatInfo,
	...
	if modeJSON {
		...
		if flagDetail || flagTrace {
			...
			fmt.Fprintf(str, "%s:%s", finfo.message, finfo.err)
		}
	}
